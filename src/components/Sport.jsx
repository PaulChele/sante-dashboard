...
const PROGRAMME = {
  1: { jour: 'Lundi', focus: 'Hamstring', calories: 380, blocs: [
  ...
  2: { jour: 'Mardi', focus: 'Dos', calories: 360, blocs: [
  ...
  3: { jour: 'Mercredi', focus: 'Épaules / Fessiers', calories: 390, blocs: [
  ...
  4: { jour: 'Jeudi', focus: 'Jambes', calories: 420, blocs: [
  ...
  5: { jour: 'Vendredi', focus: 'Poitrine', calories: 370, blocs: [
  ...
  return (
    <div className="module">
      ...
      <div className="module-header">
        <div className="module-title">BIGG — {seance.focus}</div>
        <div className="module-sub">Programme du {seance.jour.toLowerCase()}</div>
      </div>
      ...
      {seance.blocs.length === 0 ? (
        <div className="empty">
          {aujourdhui === 6 ? 'Marche active — profitez de l\'air libre' : 'Repos — récupération'}
        </div>
      ) : (
            ...
            {ouvert === i && (
              <div className="bloc-desc">
                {bloc.exercices.split(', ').map((ex, j) => (
                  <div key={j} style={{padding: '0.25rem 0', borderBottom: '1px solid var(--border)'}}>{ex}</div>
                ))}
              </div>
            )}
          </div>
      ...
      {(seance.midi || seance.soir) && (
        <div className="card" style={{marginTop: '1.5rem'}}>
          <div className="card-title">Repas du jour</div>
          {seance.midi && <div className="repas-row"><span className="repas-label">Midi</span><span>{seance.midi}</span></div>}
          {seance.soir && <div className="repas-row"><span className="repas-label">Soir</span><span>{seance.soir}</span></div>}
        </div>
      )}
    </div>
  )
